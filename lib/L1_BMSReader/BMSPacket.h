...
class BMSPacket
{
	public:
		...
		struct params_t
		{
			...
			uint16_t voltage;			// Напряжение на всей АКБ, мВ.
	...
	private:
		...
		uint16_t _CRC()
		{
			...
			for(uint8_t i = 4; i < sizeof(params) - 2; ++i)
			{
				result += *((uint8_t*)(&params) + i);
			}
		...
		/*
			Метод конвертирует полученные данные в единый вид: Напряжение в мВ, Ток в А, время в Сек....
			Модифицируется исходная структура, поэтому после crc будет не валидным.
		*/
		void _Conversion()
		{
			// ...
